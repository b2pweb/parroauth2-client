...
/**
 * Class AuthorizationCodeFlowWithOpenIdTest
 */
class AuthorizationCodeFlowWithOpenIdTest extends FunctionalTestCase
{
    /**
     * @var Client
     */
    ...
    public function test_handleAuthorizationResponse_functional()
    {
        ...
        $this->assertInstanceOf(TokenResponse::class, $tokens);
        $this->assertNotEmpty($tokens->accessToken());
        $this->assertEquals('bearer', $tokens->type());
        $this->assertEquals(['openid', 'email', 'name'], $tokens->scopes());
        $this->assertFalse($this->session->has('authorization'));
        $this->assertEquals('http://localhost:5000', $tokens->idToken()->issuer());
        $this->assertEquals('test', $tokens->idToken()->audience());
    }
